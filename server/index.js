...
app.post("/api/report", async (req, res) => {
  try {
    ...
    // If parsing failed (often due to truncation), retry once with a shorter, strict JSON-only prompt.
    if (!parsed) {
      const shortPrompt = [
        prompt,
        "",
      ...
      if (retry.raw) lastRaw = retry.raw;
      if (retry.json) parsed = retry.json;
    }
